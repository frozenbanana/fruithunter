...
cbuffer colorInfo : register(b9) {
	float4 cb_color_flat;
	float4 cb_color_tilt;
	float2 cb_intensityRange;
};
...
float4 main(PS_IN ip) : SV_TARGET {
	...
	float tilt = specialLerp(dotN, 0.70f, 0.70f);
	float intensity = lerp(cb_intensityRange.x, cb_intensityRange.y, ip.colorIntensity);
	float3 baseColor =
		lerp(cb_color_tilt.rgb, cb_color_flat.rgb, float3(1, 1, 1) * tilt) * intensity;
	...
	if (brush_dist < cb_brush_radius) {
		...
		finalColor = float3(lerp(baseColor, brush_color, smoothedMix));
	}
	else if (brush_dist < cb_brush_radius + brush_edgeThickness) {
		finalColor = lerp(baseColor, brush_color, cb_brush_strength);
	}
	return float4(lighting(ip.PosW, ip.Normal, finalColor, shade), 1);
