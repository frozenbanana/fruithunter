...
float shading_cell(float val, float v_min, float v_max, int divisions) {
	return floor((((val - v_min) / (v_max - v_min)) * divisions)) / divisions;
}
...
float4 main(PS_IN ip) : SV_TARGET {
	...
	// move backfaced texels to be on plane
	bool ignoreShade = false;
	bool ignoreShading = false;
	if (dot(toCamera, normal) < 0) {
		// backfaced texel
		// move to fill clipped geometry
		// find plane intersection
		// eq. d = ((p0-l0) dot n) / (l dot n)
		float d = dot(plane_point - position, plane_normal) / dot(toCamera, plane_normal);

		//set position and normal to fill area
		float3 oldP = position;
		float3 newP = position + toCamera * d;
		normal = -normal;
		pixelBaseColor = abs(newP - oldP);
		pixelBaseColor = plane_color;
		ignoreShade = true;
		ignoreShading = true;
	}
	...
	if (diffuseTint > 0.0) {
		...
		float3 toCam = normalize(position - camera_position.xyz);
	...
	if (ignoreShading) {
		return float4(pixelBaseColor, clipAlpha);
	}
	else {
		float shade = 1;
		if (!ignoreShade)
			shade = texSampleGrease(
				texture_shadowMap, cb_shadowMapRes, ip.ShadowPosH.xy, ip.ShadowPosH.z, position)
						.r;
