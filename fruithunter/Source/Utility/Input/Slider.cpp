...
float2 Slider::getSliderPos() const {
	return m_sliderOffset + float2((m_value * 240.f) - 120.f, 0);
}

void Slider::_draw(const Transformation2D& source) {
	float2 sliderValuePos = getSliderPos();
	m_img_background.setPosition(float2(150.f, 0.f));
	m_img_background.draw(source);

	m_img_grabber.setPosition(sliderValuePos);
	m_img_grabber.setColor(m_colour);
	m_img_grabber.setScale(0.08f);
	m_img_grabber.draw(source);

	m_textRenderer.setScale(0.4f);
	m_textRenderer.setAlignment(HorizontalAlignment::AlignRight, VerticalAlignment::AlignCenter);
	m_textRenderer.setPosition(float2(0.f));
	m_textRenderer.setText(m_label + ":");
	m_textRenderer.draw(source);

	m_textRenderer.setScale(0.15f);
...
Slider::Slider() {
	...
	m_textRenderer.setFont("lato.spritefont");
}
...
float2 Slider::getLocalSize() const { return m_img_background.getSize(); }

float Slider::getValue() { return m_value; }

void Slider::setValue(float value) { m_value = value; }
...
bool Slider::update() {
	...
	float2 pos = float2::Transform(m_img_grabber.getPosition(), getMatrix());
